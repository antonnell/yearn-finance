...
class Store {
  ...
  configure = async(payload) => {
    try {
      ...
      async.map(protocolsJSON.protocols, (protocol, callback) => {
        ...
        const filteredCoverObjects = protocol.coverObjects.filter((obj, index) => {
          return moment(expires[index]*1000) > now
        })

        async.map(filteredCoverObjects, (coverObject, callbackInner) => {
          ...
          let claimPoolData = poolDataArr.filter((data) => {
            ...
            return {
              ...
              daiInPool: BigNumber(data[1].poolId.tokens.find((token) => token.address.toLowerCase() === daiAddress.toLowerCase()).balance).toNumber(),
              covTokenBalance: BigNumber(data[1].poolId.tokens.find((token) => token.address.toLowerCase() === claimAddress.toLowerCase()).balance).toNumber(),
              covTokenWeight: BigNumber(data[1].poolId.tokens.find((token) => token.address.toLowerCase() === claimAddress.toLowerCase()).denormWeight).div(data[1].poolId.totalWeight).toNumber()
            }
          })

          if(claimPoolData.length > 0) {
            claimPoolData = claimPoolData.sort((a, b) => {
              return b.liquidity !== null ? BigNumber(b.liquidity).minus(a.liquidity) : -9999999999
            })[0]
          } else {
            claimPoolData = {
              price: 0,
          ...
          let noClaimPoolData = poolDataArr.filter((data) => {
            ...
            return {
              ...
              liquidity: data[1].poolId.liquidity,
              daiInPool: BigNumber(data[1].poolId.tokens.find((token) => token.address.toLowerCase() === daiAddress.toLowerCase()).balance).toNumber(),
              covTokenBalance: BigNumber(data[1].poolId.tokens.find((token) => token.address.toLowerCase() === noClaimAddress.toLowerCase()).balance).toNumber(),
              covTokenWeight: BigNumber(data[1].poolId.tokens.find((token) => token.address.toLowerCase() === noClaimAddress.toLowerCase()).denormWeight).div(data[1].poolId.totalWeight).toNumber()
            }
          })
  ...
  getCoverBalances = async (payload) => {
    ...
    async.map(coverProtocols, async (protocol, callback) => {

      async.map(protocol.poolData, async(poolData, callbackInner) => {
        const claimAsset = await this._getInfo(web3, account, poolData.claimAddress, poolData.claimPoolData.address)
        const noClaimAsset = await this._getInfo(web3, account, poolData.noClaimAddress, poolData.claimPoolData.address)
        const collateralAsset = await this._getInfo(web3, account, poolData.collateralAddress, poolData.claimPoolData.address)
  ...
  _getInfo = async (web3, account, asset, spender) => {
    ...
    try {
      ...
      balance = BigNumber(balance).div(bnDec(decimals)).toFixed(decimals)

      let allowance = await erc20Contract.methods.allowance(account.address, spender).call()
      allowance = BigNumber(allowance).div(bnDec(decimals)).toFixed(decimals)
  ...
  buyCover = async (payload) => {
    ...
    const { asset, collateral, amount, amountOut, pool, gasSpeed } = payload.content

    this._callSwapExactAmountIn(web3, collateral, asset, amount, amountOut, account, pool, gasSpeed, (err, res) => {
      if(err) {
  ...
  sellCover = async (payload) => {
    ...
    this._callSwapExactAmountIn(web3, asset, collateral, amount, amountOut, account, pool, gasSpeed, (err, res) => {
      if(err) {
        return this.emitter.emit(ERROR, err);
      }
  ...
  approveCover = async (payload) => {
    ...
    this._callApproveCover(web3, poolAddress, asset, amount, gasSpeed, account, (err, approveResult) => {
      if(err) {
        return this.emitter.emit(ERROR, err);
      }
  ...
  _callApproveCover = async (web3, poolAddress, asset, amount, gasSpeed, account, callback) => {
    ...
    if(amount === 'max') {
      ...
      amountToSend = BigNumber(amount).times(10**asset.decimals).toFixed(0)
    }

    const gasPrice = await stores.accountStore.getGasPrice(gasSpeed)

    this._callContract(web3, tokenContract, 'approve', [poolAddress, amountToSend], account, gasPrice, GET_COVER_BALANCES, callback)
  }
}
