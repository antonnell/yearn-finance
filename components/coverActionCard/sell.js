...
export default function Sell({ coverProtocol }) {
  ...
  const setAmountPercent = percent => {
    ...
    const value = BigNumber(coverProtocol.poolData[0].claimAsset.balance)
      .times(percent)
      .div(100)
      .toFixed(
        coverProtocol.poolData[0].claimAsset.decimals,
  ...
  const onSell = () => {
    if (
      ...
      isNaN(amount) ||
      amount <= 0 ||
      BigNumber(amount).gt(coverProtocol.poolData[0].claimAsset.balance)
    ) {
      setAmountError(true);
      return false;
    }
    ...
    <div className={classes.buyContainer}>
      <div className={classes.textField}>
        <div className={classes.balances}>
          ...
          >
            Balance:{" "}
            {!coverProtocol.poolData[0].claimAsset.balance ? (
              <Skeleton />
            ) : (
              formatCurrency(coverProtocol.poolData[0].claimAsset.balance)
            )}
          </Typography>
        </div>
        <TextField
      ...
      {account && account.address && (
        <div className={classes.actionButton}>
          {(amount === "" ||
            BigNumber(coverProtocol.poolData[0].claimAsset.allowance).gte(
              amount
            )) && (
            <Button
              fullWidth
          ...
          {amount !== "" &&
            ...
            (!coverProtocol.poolData[0].claimAsset.allowance ||
              BigNumber(coverProtocol.poolData[0].claimAsset.allowance).eq(0) ||
              BigNumber(coverProtocol.poolData[0].claimAsset.allowance).lt(
                amount
              )) && (
              <React.Fragment>
                <Button
