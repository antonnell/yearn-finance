...
function TransactionHeader({ tx }) {
  return (
    <div className={ classes.txRow }>
      ...
      <div className={ classes.txCell}>
        <Typography variant='h5' align='right'>Time</Typography>
      </div>
      <div className={ classes.txCellAddress}>
        <Typography variant='h5' align='right'>TX Hash</Typography>
      </div>
    </div>
  )
}
...
function TransactionEvent({ tx, vault }) {
  ...
  return (
    <div className={ classes.txRow }>
      ...
      <div className={ classes.txCell}>
        <Typography variant='h5' align='right'>{ formatCurrency( BigNumber(tx.amount).div( bnDec(vault.decimals) ) ) } { vault.displayName }</Typography>
      </div>
      <div className={ classes.txCell}>
        <Typography variant='h5' align='right'>{ moment(tx.timestamp*1000).fromNow() }</Typography>
      </div>
      <div className={ classes.txCellAddress} onClick={ onAddressClicked }>
        <Typography variant='h5' align='right'>{ formatAddress(tx.transactionAddress) }</Typography>
      </div>
    </div>
  )
}
