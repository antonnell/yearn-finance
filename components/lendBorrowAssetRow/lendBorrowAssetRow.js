...
function LendBorrowAssetDetails({ lendingAsset, account, lendingBorrow, lendingSupply, lendingBorrowLimit }) {
  ...
  const onRepay = () => {
    ...
    stores.dispatcher.dispatch({ type: REPAY_LEND, content: { amount: repayAmount, lendingAsset: lendingAsset, gasSpeed: gasSpeed } })
  }

  const onApprove = () => {
    setLoading(true)
    stores.emitter.on(APPROVE_LEND_RETURNED, approveReturned);
    stores.dispatcher.dispatch({ type: APPROVE_LEND, content: { lendingAsset: lendingAsset, amount: repayAmount, gasSpeed: gasSpeed } })
  }
  ...
  const setRepayAmountPercent = (percent) => {
    ...
    const amount = BigNumber(lendingAsset.borrowBalance).times(percent).div(100).toFixed(lendingAsset.tokenMetadata.decimals)
  ...
  if(lendingBorrowLimit === 0) {
    ...
    theLimitUsed = (lendingBorrow-(repayAmount*lendingAsset.price))*100/lendingBorrowLimit
  } else {
    ...
    <Paper elevation={ 0 } square className={ classes.assetActions }>
      ...
      <div className={ classes.actionsContainer }>
        ...
        <div className={classes.tradeContainer}>
          <div className={ classes.inputTitleContainer }>
            <div className={ classes.inputTitle }>
              <Typography variant='h5' noWrap>Repay</Typography>
            </div>
            <div className={ classes.balances }>
              <Typography variant='h5' onClick={ () => { setRepayAmountPercent(100) } }  className={ classes.value } noWrap>{ 'Wallet: ' + formatCurrency(lendingAsset.tokenMetadata.balance) } { lendingAsset.tokenMetadata.symbol }</Typography>
            </div>
          </div>
          <TextField
            fullWidth
          ...
          <div className={ classes.scaleContainer }>
            <Button
              ...
              disabled={ loading }
              color="primary"
              onClick={ () => { setRepayAmountPercent(25) } }>
            ...
            <Button
              ...
              disabled={ loading }
              color="primary"
              onClick={ () => { setRepayAmountPercent(50) } }>
            ...
            <Button
              ...
              disabled={ loading }
              color="primary"
              onClick={ () => { setRepayAmountPercent(75) } }>
            ...
            <Button
              ...
              disabled={ loading }
              color="primary"
              onClick={ () => { setRepayAmountPercent(100) } }>
          ...
          <div className={ classes.buttons }>
            { (repayAmount !=='' && BigNumber(repayAmount).gt(0) && (!lendingAsset.tokenMetadata.allowance || BigNumber(lendingAsset.tokenMetadata.allowance).eq(0) || BigNumber(lendingAsset.tokenMetadata.allowance).lt(repayAmount))) && (
              <React.Fragment>
                <Button
                  fullWidth
                  disableElevation
            ...
            { (repayAmount === '' || BigNumber(lendingAsset.tokenMetadata.allowance).gte(repayAmount)) && (
              <Button
                fullWidth
                disableElevation
                variant='contained'
                color='primary'
                size='large'
                disabled={ loading || BigNumber(lendingAsset.borrowBalance).eq(0) || !repayAmount || BigNumber(repayAmount).gt(lendingAsset.supplyBalance) }
                onClick={ onRepay }
                >
                { loading && <CircularProgress size={20} /> }
                { !loading && <Typography className={ classes.buttonText } variant={ 'h5'} >
                  { BigNumber(lendingAsset.borrowBalance).eq(0) && 'No Balance to Repay' }
                  { BigNumber(lendingAsset.borrowBalance).gt(0) && BigNumber(repayAmount).gt(BigNumber(lendingAsset.tokenMetadata.balance)) && 'Insufficient Balance' }
                  { BigNumber(lendingAsset.borrowBalance).gt(0) && BigNumber(repayAmount).gt(BigNumber(lendingAsset.borrowBalance)) && 'Insufficient Borrowed' }
                  { BigNumber(lendingAsset.borrowBalance).gt(0) && (!repayAmount || (BigNumber(repayAmount).lte(BigNumber(lendingAsset.tokenMetadata.balance)) && BigNumber(repayAmount).lte(BigNumber(lendingAsset.borrowBalance)))) && 'Repay' }
                </Typography> }
              </Button>
            )}
          </div>
...
export default function LendBorrowAssetRow({ lendingAsset, account, lendingBorrow, lendingSupply, lendingBorrowLimit }) {
    ...
    <Accordion className={ classes.removePaper } elevation={0} square expanded={ expanded } onChange={ () => { handleExpand() } }>
      <AccordionSummary  elevation={0} className={ classes.lendingRow }>
        ...
        <div className={ classes.lendValueCell}>
          <Typography variant='h5' className={ classes.balance } noWrap>{ formatCurrency(lendingAsset.borrowBalance) } { lendingAsset.tokenMetadata.symbol }</Typography>
        </div>
      ...
      <AccordionDetails>
        <LendBorrowAssetDetails lendingAsset={ lendingAsset } lendingBorrow={ lendingBorrow } lendingSupply={ lendingSupply } lendingBorrowLimit={ lendingBorrowLimit } />
      </AccordionDetails>
    </Accordion>
  )
}
