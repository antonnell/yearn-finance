...
import classes from './cdpSuppliedGraph.module.css'
...
export default function LendBorrowGraph({ assets }) {
  ...
  const data = assets.filter((asset) => {
    return BigNumber(asset.collateralDolar).gt(0)
  }).map((asset) => {
    return {
      ...
      value: parseFloat(asset.collateralDolar),
      collateral: parseFloat(asset.collateral)
    }
  })
