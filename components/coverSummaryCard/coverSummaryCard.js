      ...
      <div className={ classes.coverTitleContainer }>
        <div className={ classes.coverOutline } >
          <div className={ classes.coverLogo }>
            { !coverProtocol ? <Skeleton /> : <img src={ getLogoForProtocol(coverProtocol.name) } onError={(e)=>{e.target.onerror = null; e.target.src="/tokens/unknown-logo.png"}} alt='' width={ 60 } height={ 60 } /> }
          </div>
        </div>
        <div className={ classes.coverTitle }>
          <Typography variant='h1'>{ !coverProtocol ? <Skeleton /> : `${coverProtocol.protocolDisplayName}` }</Typography>
          <Typography>{ !coverProtocol ? <Skeleton /> : `Claim Tokens` }</Typography>
        </div>
      </div>
      <div>
        <div className={ classes.coverDetailGroup }>
          <Typography className={ classes.coverDetailTitle }>Expiration Date</Typography>
          <Typography className={ classes.coverDetailValue }>{ moment(coverProtocol.expires[0]*1000).format('Do MMM YYYY') }</Typography>
        </div>
        <div className={ classes.coverDetailGroup }>
