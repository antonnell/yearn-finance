...
function Stats({ changeTheme }) {
  ...
  const onPoolSelectChange = (event) => {
    ...
    setLoading(true)
    stores.dispatcher.dispatch({ type: GET_MAX, content: { address: theOption[0].address } })
  }
    ...
    return (
      <MenuItem key={option.id} value={option.symbol} className={ classes.assetSelectMenu }>
        <div className={ classes.poolSelectOption }>
          <img className={ classes.poolIcon } src={ `https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/${web3.utils.toChecksumAddress(option.address)}/logo.png` } width={ 30 } height={ 30 } />
      ...
      <div className={ classes.ltvContainer }>
        <Paper elevation={0} className={ classes.searchContainer }>
          <div className={ classes.textField }>
            <div className={ classes.inputTitleContainer }>
              <div className={ classes.inputTitle }>
                <Typography variant='h5' noWrap>Select Asset</Typography>
              </div>
            </div>
            <TextField
              select
                ...
                renderValue: (option) => {
                  ...
                  return (
                    <div className={ classes.assetSelectIconName }>
                      <div className={ classes.poolSelectOption }>
                        <img className={ classes.poolIcon } src={ `https://raw.githubusercontent.com/trustwallet/assets/master/blockchains/ethereum/assets/${web3.utils.toChecksumAddress(theOption[0].address)}/logo.png` } width={ 30 } height={ 30 } />
                        <Typography variant='h5'>{ theOption[0].symbol }</Typography>
                      </div>
                    </div>
                  )
                }
              ...
              placeholder={ 'Select' }
            >
              { assets ? assets.map((a) => { return renderAssetOption(web3, a) }) : '' }
            </TextField>
          </div>
          <div>
          ...
          <div className={ classes.copyContainer }>
            <Typography variant='h1' className={ classes.titleSpacing }>LTV Lookup</Typography>
            <Typography variant='h5' className={ classes.lineHeightIncrease }>LTV Lookup displays how much you can borrow per $1 worth of the asset provided to the different protocols.</Typography>
          </div>
      </div>
    </Layout>
  )
